...
export function authenticationMiddleware(config: JwtSecurityConfig): RequestHandler {
  return (req: Request, res: Response, next: NextFunction): void => {
    ...
    try {
      ...
      // First try TON JWT (TonConnect flow) - defaults to JWT secret if TON_JWT_SECRET not provided
      const tonSecret = process.env.TON_JWT_SECRET || config.secret;
      // logger.info('ton secret log', {
      //   token: token,
      //   tonSecret: tonSecret,
      //   issuer: process.env.TON_JWT_ISSUER || config.issuer || 'panoramablock-ton',
      //   audience: process.env.TON_JWT_AUDIENCE || config.audience || 'panoramablock'
      // })
      if (tonSecret) {
        try {
